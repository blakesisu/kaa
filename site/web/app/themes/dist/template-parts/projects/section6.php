<section class="proj-section6">
  <div class="proj-projects__tri-container">

    <div class="proj__tri-projects">
      <div class="proj-projects__project proj-project__tri">
        <picture class="proj-project__picture">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/4thstreet-lg.jpg" media="(max-width: 700px)">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/4thstreet-lg.jpg">
          <img class="proj-project__img" srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/4thstreet-lg.jpg" alt="">
        </picture>
        <div class="proj-project__description">
          <h3 class="proj-text-mini__header">4TH WALKSTREET</h3>
        </div>
      </div>

      <div class="proj-projects__project proj-project__tri">
        <picture class="proj-project__picture">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/mobile/23rd-sm.jpg" media="(max-width: 700px)">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/23rd-lg.jpg">
          <img class="pd-related-project__img proj-project__img" srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/23rd-lg.jpg" alt="">
        </picture>
        <div class="proj-project__description">
          <h3 class="proj-text-mini__header">23RD & THE STRAND</h3>
        </div>
      </div>

      <div class="proj-projects__project proj-project__tri">
        <picture class="proj-project__picture">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/mobile/emeraldbay-sm.jpg" media="(max-width: 700px)">
          <source srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/emeraldbay-lg.jpg">
          <img class="pd-related-project__img proj-project__img" srcset="<?php bloginfo('template_url'); ?>/images/projects/desktop/emeraldbay-lg.jpg" alt="">
        </picture>
        <div class="proj-project__description">
          <h3 class="proj-text-mini__header">EMERALD BAY</h3>
        </div>
      </div>
    </div>
  </div>
</section>
